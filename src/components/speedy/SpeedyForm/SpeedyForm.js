...
export default class SpeedyForm extends Speedy {
    ...
    render(h) {
        return h(resolveComponent('ElForm'), {
            model: this.form,
    ...
    renderFormItems() {
        ...
        if (this.formLayout.value && this.formLayout.value.length) {
            ...
            return h(resolveComponent('ElRow'), {}, () => this.formItems.value.map(item => {
                return h(resolveComponent('ElCol'), { span: item.span || 24 }, () => {
                    if (item.type === EnumFormItemType.Custom) {
                        return slots.default?.();
                    }
                    else {
                        return h(resolveComponent('ElFormItem'), { label: item.label, prop: item.prop }, renderSpecifyItem(item, this.form.value, this.eventsStore, this.store));
                    }
                });
            }));
        }
    }
    renderFormLayout(layout) {
        return this.formLayout.value.map(l => {
            if (l.type === 'row') {
                return h(resolveComponent('ElRow'), {
                    gutter: l.gutter || 0
                ...
                if (temp) {
                    if (temp.slot) {
                        return h(resolveComponent('ElCol'), {
    ...
    renderFormItem(item) {
        return h(resolveComponent('ElFormItem'), {
            label: item.label,
            prop: item.prop
        }, renderSpecifyItem(item, this.form.value, this.events, this.store));
    }
}
//# sourceMappingURL=SpeedyForm.js.map